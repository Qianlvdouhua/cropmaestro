...
CropMonster orchestrates n8n agents with MySQL and optional backend HTTP services. The open-source demo uses **commercial rice** (`bus_rice_data`), not the national germplasm `rice_data` table.
...
| Table | Role |
| --- | --- |
| `bus_rice_data` | Commercial variety records (no 共享类型) |
